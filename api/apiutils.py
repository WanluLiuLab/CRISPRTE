...
class PGSQL:
    @staticmethod
    def connect(dbname):
        return psycopg2.connect(f'dbname={dbname} user=postgres password={os.getenv("pgps")}')
...
def query_mismatch_annotation(pcursor, **args):
    ...
    result = {}
    # azimuth_score = calculate_azimuth(list(map(lambda x: x[5][-4:] + PGSQL.select_gseq_table2_gid(pcursor, x[0]) + x[4] + x[6][:3], targetTE_gids)))
    for ind, i in enumerate(targetTE_gids):
        result[i[0]] = {}
        result[i[0]]['pos'] = i[1]
        result[i[0]]['moreno'] = i[2]
        result[i[0]]['azimuth'] = 0
        result[i[0]]['pam'] = i[4]
